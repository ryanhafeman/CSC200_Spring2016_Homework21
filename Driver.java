...
public class Driver 
{
	...
	static void mergeSortHelper(int[] ar, int begin, int end)
	{
		...
		if(begin != end)
		{
			...
			int[] newArray = new int[rightEnd-leftBegin +1]; //new array to hold merged values
			int currLeft = leftBegin; //1st bucket in left array is the same as var leftBegin
			int currRight = rightBegin; //1st bucket in right array is the same as var rightBegin
			for(int currentBucket = 0; currentBucket < newArray.length; currentBucket = currentBucket++) //go through all buckets of newArray
				
				//if/then comparisons of right and left side buckets to merge them 
			{
